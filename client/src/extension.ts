...
function enable () {
  ...
  let disabledFolders = folders.filter((folder) => {
    return !Workspace.getConfiguration('eslint', folder.uri).get('enable', true)
  })
...
function shouldBeValidated (textDocument: TextDocument): boolean {
  ...
  for (let item of validate) {
    ...
    if (ValidateItem.is(item) && item.language === textDocument.languageId) {
      return true
    }
  }
...
export function activate (context: ExtensionContext) {
  ...
  let notValidating = () => {
    return Window.showInformationMessage('ESLint is not running. By default only JavaScript files are validated. If you want to validate other file types please specify them in the \'eslint.validate\' setting.')
  }
