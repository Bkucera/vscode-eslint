...
function getFixes (textDocument: TextDocument): TextEdit[] {
  ...
  if (edits) {
    ...
    return fixes.getOverlapFree().filter((fix) => {
      return !!fix.edit
    }).map(createTextEdit)
  }
...
messageQueue.registerRequest(CodeActionRequest.type, (params) => {
  ...
  return resolveSettings(textDocument).then((settings) => {
    ...
    if (result.length > 0) {
      let sameProblems: Map<string, FixableProblem[]> = new Map<string, FixableProblem[]>(allFixableRuleIds.map<[string, FixableProblem[]]>((s) => {
        return [s, []]
      }))
      ...
      sameProblems.forEach((same, ruleId) => {
        if (same.length > 1) {
          ...
          same.map(createTextEdit).forEach((edit) => {
      ...
      if (all.length > 1) {
        ...
        all.map(createTextEdit).forEach((edit) => {
          return allTextChange.add(edit)
        })
